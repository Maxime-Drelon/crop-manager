class WeatherDataManager {
	...
	public async getCurrentWeather({ city }: { city: string }): Promise<Map<string, any> | void> {
		try {
			const response = await fetch(`${this.baseUrl}?q=${city}&appid=${this.APIKey}&lang=fr`, {
				method: 'GET'
			});
	...
	public setIcon(): void {
		(
			document.getElementById('wicon') as HTMLImageElement
		).src = `http://openweathermap.org/img/wn/${this.currentWeather['weather'][0]['icon']}@2x.png`;
	}
}
